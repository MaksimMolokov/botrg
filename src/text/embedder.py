...
class OllamaHTTPEmbeddings(Embeddings):
    ...
    def __init__(self, endpoint: str, model: str):
        ...
        ep = endpoint.rstrip("/")
        # Ollama 0.12+ использует /api/embed, а не /api/embeddings
        if not ep.endswith("/api/embed"):
            ep = f"{ep}/api/embed"
        self._endpoint = ep
        self._model = model
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        try:
            # Ollama 0.12+ API использует поле "input" вместо "prompt"
            ...
            data = response.json()
            # Ответ приходит в формате {"embeddings": [[...]]}
            ...
            if isinstance(embeddings, list) and embeddings:
                ...
                if isinstance(vector, list):
                    ...
                    return vector
            # Фолбэк для старых версий Ollama
