...
from ..config_store import ConfigStore, get_admin_users


def _is_user_admin(user_id: int, store: ConfigStore) -> bool:
    """Проверяет, является ли пользователь администратором."""
    admins = get_admin_users(store)
    if not admins:
        # Если список админов пуст - режим без ограничений
        # В этом режиме всем разрешаются ВСЕ операции, включая удаление БД
        return True
    
    # Если есть админы, проверяем пользователя
    return str(user_id) in admins


def _check_user_permission(user_id: int, store: ConfigStore, operation: str = "") -> bool:
    """Проверяет разрешение пользователя на выполнение операции.
    ...
    """
    # Если список администраторов пуст - разрешаем всем (режим без ограничений)
    admins = get_admin_users(store)
    if not admins:
        return True
    
    # Если есть админы, проверяем пользователя
    return _is_user_admin(user_id, store)
...
class DocumentIngestionService:
    ...
    def clear_store(self, user_id: int = 0) -> bool:
        """Очищает векторное хранилище документов.
        
        Args:
            user_id: ID пользователя Telegram для проверки прав доступа
            
        Returns:
            True если операция выполнена, False если нет прав доступа
        """
        store = ConfigStore(os.environ.get("APP_CONFIG_JSON", "/app/data/app-config.json"))
        if not _check_user_permission(user_id, store, "clear_store"):
            self._logger.warning("Пользователь %d попытался очистить хранилище без прав администратора", user_id)
            return False
    ...
    async def ingest_from_message(self, message: Message, bot: Bot) -> None:
        ...
        # Проверка прав доступа для загрузки документов
        user_id = message.from_user.id if message.from_user else 0
        store = ConfigStore(os.environ.get("APP_CONFIG_JSON", "/app/data/app-config.json"))
        
        if not _check_user_permission(user_id, store, "ingest_document"):
            await message.answer(text="Ошибка: У вас нет прав для загрузки документов. Обратитесь к администратору.")
            self._logger.warning("Пользователь %d попытался загрузить документ без прав администратора", user_id)
            return
    ...
    async def ingest_photo_from_message(self, message: Message, bot: Bot) -> None:
        ...
        # Проверка прав доступа для загрузки изображений
        user_id = message.from_user.id if message.from_user else 0
        store = ConfigStore(os.environ.get("APP_CONFIG_JSON", "/app/data/app-config.json"))
        
        if not _check_user_permission(user_id, store, "ingest_photo"):
            await message.answer(text="Ошибка: У вас нет прав для загрузки изображений. Обратитесь к администратору.")
            self._logger.warning("Пользователь %d попытался загрузить фото без прав администратора", user_id)
            return
