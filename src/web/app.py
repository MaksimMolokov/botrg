...
class SettingsWebApp:
    ...
    def _routes(self) -> None:
        ...
        self._app.router.add_post("/api/admin/users", self._add_user)
        self._app.router.add_delete("/api/admin/users/{user_id}", self._remove_user)
        self._app.router.add_get("/api/admin/users", self._get_users)
    ...
    async def _set_config(self, request: web.Request) -> web.Response:
        """Сохранить настройки конфигурации с частичным обновлением."""
        ...
        # Если сохраняются общие настройки, автоматически включаем поля RBAC
        if any(key in body for key in ['EMBEDDINGS_MODEL', 'OPENAI_BASE_URL', 'OPENAI_API_KEY', 
    ...
    async def _add_user(self, request: web.Request) -> web.Response:
        """Добавить пользователя (админа или обычного)."""
        body = await request.json()
