...
class TelegramBotApplication:
    ...
    def __init__(self, settings: Settings):
        ...
        # ConfigStore для проверки прав администратора
        import os
    ...
    async def _on_message(self, message: Message) -> None:
        ...
        if message.document or message.photo:
            # Проверка прав администратора для загрузки документов
            if not self._is_admin(user_id):
                await message.answer("Только администраторы могут загружать документы.")
                return
...
async def main() -> None:
    ...
    if "ALLOWED_USER_IDS" in json_config and json_config["ALLOWED_USER_IDS"]:
        settings.regular_users = json_config["ALLOWED_USER_IDS"]
