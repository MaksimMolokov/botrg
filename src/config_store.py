...
def parse_allowed_users(users_str: str) -> List[str]:
    """Парсит строку с ID пользователей через запятую, возвращает список строк."""
    if not users_str:
        return []
    return [u.strip() for u in users_str.split(",") if u.strip()]


def get_admin_users(store: ConfigStore) -> List[str]:
    """Возвращает список администраторов."""
    data = store.load()
    return parse_allowed_users(data.get("ALLOWED_ADMIN_IDS", ""))


def get_regular_users(store: ConfigStore) -> List[str]:
    """Возвращает список обычных пользователей."""
    data = store.load()
